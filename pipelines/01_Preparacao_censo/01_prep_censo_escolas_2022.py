"""PIPELINE 01 (2022): PREPARAÇÃO DA BASE DE ESCOLAS (CENSO 2022 + SEDUC 2022)

Camada: Bronze -> Silver
Entradas:
  - data/raw/1. SEDUC-SP — SARESP & Cadastro de Escolas/B - Tabela de Ligação/ENDERECO_ESCOLAS_2022_0.csv
  - data/raw/3. INEP — Censo Escolar da Educação Básica (Infraestrutura)/2022 microdados_censo_escolar/dados/microdados_ed_basica_2022.csv
Saída:
  - data/silver/01_escolas_base_2022.parquet
"""
...
ARQUIVO_SAIDA = PASTA_SILVER / "01_escolas_base_2022.parquet"

print("=================================================================")
print("INICIANDO PIPELINE 01 (2022): PREPARAÇÃO DA BASE DE ESCOLAS")
print("=================================================================\n")

# 1. PROCESSAMENTO SEDUC 2022
print("1. Processando Cadastro SEDUC 2022...")
colunas_seduc = [
    "NOMEDEP",
    "DE",
    "MUN",
...
print(f"   -> Escolas estaduais SEDUC mapeadas em 2022: {len(df_seduc):,}")

# 2. PROCESSAMENTO CENSO 2022
print("\n2. Processando Microdados do Censo 2022...")
colunas_censo = [
    "CO_ENTIDADE",
    "SG_UF",
    "TP_DEPENDENCIA",
...
print(
    f"   -> Escolas elegíveis do Ensino Médio no Censo 2022: {len(df_censo):,}"
)

# 3. JUNÇÃO RELACIONAL
print("\n3. Realizando junção relacional (SEDUC + Censo 2022)...")
df_silver_2022 = pd.merge(df_seduc, df_censo, on="CO_ENTIDADE", how="inner")
print(
    f"   -> Total de escolas na base final Silver 2022: {len(df_silver_2022):,}"
)

# 4. QUALITY GATES
print("\n4. Executando Quality Gates de Validação...")
assert df_silver_2022["CODESC"].isna().sum() == 0, "Erro: CODESC nulos!"
assert (
    df_silver_2022["CO_ENTIDADE"].isna().sum() == 0
), "Erro: CO_ENTIDADE nulos!"
assert df_silver_2022["CODESC"].duplicated().sum() == 0, "Erro: CODESC dup!"
assert (
    df_silver_2022["CO_ENTIDADE"].duplicated().sum() == 0
), "Erro: CO_ENTIDADE dup!"
assert list(df_silver_2022["CODESC"].str.len().unique()) == [
    6
], "Erro: tamanho CODESC!"
assert list(df_silver_2022["CO_ENTIDADE"].str.len().unique()) == [
    8
], "Erro: tamanho CO_ENTIDADE!"
print("   [PASS] Todos os Quality Gates passaram com sucesso!")

# 5. EXPORTAÇÃO
df_silver_2022.to_parquet(ARQUIVO_SAIDA, index=False, engine="pyarrow")
print(f"\n[SUCESSO] Base Silver 2022 gerada com sucesso: {ARQUIVO_SAIDA.name}")
print(
    f"   Dimensões: {df_silver_2022.shape[0]} linhas x {df_silver_2022.shape[1]} colunas\n"
)